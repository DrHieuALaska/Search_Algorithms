...
class ACO_GraphColoring:
    ...
    def _construct(self, problem: GraphColoringProblem, tau: np.ndarray, rng: np.random.Generator, order: np.ndarray) -> np.ndarray:
        n = problem.n
        k = problem.k
        ...
        for idx in range(n):
            ...
            # heuristic: prefer colors that create fewer immediate conflicts with already-colored neighbors
        ...
        # fill any unassigned (shouldn't happen)
        mask = colors < 0
        if mask.any():
    ...
    def solve(
        ...
        **meta: Any
        ...
        # order vertices by degree (high first)
