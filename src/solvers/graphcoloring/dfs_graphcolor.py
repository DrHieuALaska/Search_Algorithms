...
class DFS_BacktrackingGraphColoring:
    ...
    This is an exact decision procedure for fixed k (or returns best found if timed out).
    """

    name = "DFS_GC"

    def __init__(self, *, time_limit_sec: float = 2.0, max_backtracks: int = 2_000_000, trace_every: int = 10000):
        if time_limit_sec <= 0:
    ...
    def solve(
        ...
        **meta: Any
        ...
        def checkpoint(curr_cost: float):
            ...
            trace_best.append(best_cost if best_sol is not None else float("inf"))
        ...
        # Backtracking with recursion stack implemented iteratively for speed
        stack: List[Tuple[int, int]] = []  # (idx_in_order, next_color_to_try)
        idx = 0
        next_color = 0
        ...
        while True:
            ...
            if iters % self.trace_every == 0:
                # compute current conflicts of partial assignment as a loose proxy
                tmp = colors.copy()
                tmp[tmp < 0] = 0
